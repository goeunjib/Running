...
$(document).ready(function () {
  ...
  $('.container').children(); // 자식 노드 탐색. 매개변수를 지정하지 않으면 같은 라인의 모든 직계 자식 노드 탐색
  ...
  $('li.part').siblings('li:last-child').children().first().css({
    borderStyle: 'dashed',
    textDecoration: 'none',
    color: 'red',
  });
  ...
  // text4 탐색
  // $('#header').siblings('ul').children('li:last-child').children('a:last-child').css({ color:'#fff', textDecoration:'none', backgroundColor:'#000'});
  // $('#header').siblings('ul').children('li:last-child').children().last().css({ color:'#fff', textDecoration:'none', backgroundColor:'#000'});

  // .find('a:last-child'); ul의 하위 노드 중 a:last-child 탐색. 반드시 *매개변수* 필요
  $('#header')
    .siblings('ul')
    .find('a:last-child')
    .css({ color: '#fff', textDecoration: 'none', backgroundColor: '#000' });
